...
class StartAll {
    ...
    ServiceDescriptor[] getServiceDescriptors() {
        ...
        def descriptors = []
        def configArg = ["${configPath}/sqrt-prv.config"]
        def codebase = "${relativeRepoPath}/sqrt-${sorcerVersion}-dl.jar ${relativeRepoPath}/sqrt-${sorcerVersion}-ui.jar sorcer-dl-${sorcerVersion}.jar jsk-dl-${riverVersion}.jar"

        descriptors << new SorcerServiceDescriptor(codebase,
                                                   policy,
                                                   "${buildLibPath}/sqrt-${sorcerVersion}-prv.jar${File.pathSeparator}${buildLibPath}/sqrt-${sorcerVersion}-ui.jar",
